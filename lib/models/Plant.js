...
module.exports = class Plant {
    ...
    static async insert({ plant_name, description, scientific_name, image, category_id, userId, on_market }) {
        const { rows } = await pool.query(
            `INSERT INTO plants (plant_name, description, scientific_name, image, category_id, users_id, on_market)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
            RETURNING *
            `,
            [plant_name, description, scientific_name, image, category_id, userId, on_market]
        );
    ...
    static async updatePlantById(plantId, userId, {plant_name, description, scientific_name, image, category_id, on_market}) {

        const { rows } = await pool.query(
            'UPDATE plants SET plant_name=$1, description=$2, scientific_name=$3, image=$4, category_id=$5, on_market=$6 WHERE id=$7 AND users_id=$8 RETURNING *',
            [plant_name, description, scientific_name, image, category_id, on_market, plantId, userId]
        );
    ...
    static async deletePlantById(id, userId) {
      ...
      static async getByOnMarket() {
        const { rows } = await pool.query(`SELECT * FROM plants WHERE on_market=true`, );
        return rows.map((row) => new Plant(row));
    }
